...
class Test(object):
    ...
    def setup_class(self):
        ...
        self.exec_sql('lxk', self.sql_yaml_data['实名绑卡'][0], self.sql_yaml_data['实名绑卡'][1].format(self.yaml_data['account']['c_account']))
        self.exec_sql('lxk', self.sql_yaml_data['实名绑卡'][0], self.sql_yaml_data['实名绑卡'][2].format(self.yaml_data['account']['c_account']))
        self.exec_sql('lxk', self.sql_yaml_data['清除交易密码'][0], self.sql_yaml_data['清除交易密码'][1].format(self.yaml_data['account']['c_account']))
    ...
    #@allure.feature('蓝薪卡')
    @allure.story('lxk_设置交易密码')
    def test_set_deal_pwd(self,get_lxk_c_headers,function,casename,type,run,url,hearders,param,sql1,sql2,sql3,asserttype,expect1,expect2,expect3):
        """
        设置交易密码
...
if __name__ == "__main__":
    pytest.main(["-s", "test_006_setDealPwd.py"])
